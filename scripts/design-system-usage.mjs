#!/usr/bin/env node
/**
 * V3 strict usage validation (Phase 3).
 *
 * Deterministic, AST-based checking of a configured consumer repository. It
 * discovers the consumer contract through the Phase 2 config-first tooling,
 ...
 *
 * This module is both a library (imported by `check-design-system-usage.mjs`)
 * and exposes helpers for focused tests.
 */
...
} from "./design-system-consumer.mjs";
import { V2_REQUIRED_COMPONENTS } from "./register-design-system.mjs";
...
export function loadUsageContext({ cwd, ignore } = {}) {
  ...
  const strict = discovered.config ? discovered.config.strict : true;
 ...
 */
export function checkUsage({ cwd, ignore } = {}) {
  const context = loadUsageContext({ cwd, ignore });
