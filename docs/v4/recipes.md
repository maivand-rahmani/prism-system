...
Практические примеры для продукта, который использует опубликованный пакет
дизайн-системы: контентная страница, форма и страница с данными. Собранный
интерфейс должен работать без чтения исходников монорепозитория — только публичный
API пакета и его манифест. Спецификация — [README.md](README.md), общий гайд —
[../guide.md](../guide.md). Историю миграции потребителя с прежнего контракта
смотрите в [архиве](../archive/README.md).
...
- **Только обязательные компоненты.** Все три рецепта работают с любой
  системой текущего контракта: используются 29 обязательных компонентов, ни один
  дополнительный
  (`Table`, `Section`, `Toast` и т. п.) не требуется. Фактический набор системы
  показывают ключи `components` в манифесте `<package>/manifest` и
  `npx prism-ds components`; `capabilities.categories` лишь сопоставляет имена
  категориям и доступность не объявляет.
...
смысловые utility-классы для композиции страницы (например, `bg-prism-surface-canvas`);
...
`check` уже включает проверку usage; с `--css` он также проверяет порядок
импортов в указанном CSS-файле. Для отдельной проверки usage используйте
`npx prism-ds check-usage --cwd .`. Команды работают офлайн и только читают
файлы продукта. Проверка синтаксиса не покрывает все визуальные переопределения:
семантические utility-классы и внешние CSS-правила требуют отдельного ревью.

Примеры с обработкой запроса и сохранением ref входят в каждый пакет:
[System A](../../packages/system-a/USAGE.md),
[System B](../../packages/system-b/USAGE.md).
